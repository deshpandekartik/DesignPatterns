...
public class GreenWest implements StreetLightsStateI
{
    private int carpass_count;
    private StretLights currentContext;
    private Results results;

    public GreenWest(StretLights contex, Results result)
    {
    ...
    /**
     * Pass a car from west side
     * @return : New state of machine
     */
    @Override
    public StreetLightsStateI pass_car()
    {
        if ( currentContext.getWest_car_count() > 0 && carpass_count < 2 )
        {
            results.storeNewResult("ACTION pass_car: Car passing from West side.");
            currentContext.WestCarPassing();
            carpass_count ++;
        }
    ...
    /**
     * Set North South East signals to RED
     */
    @Override
    public void set_otherSignals_RED() {
        results.storeNewResult("ACTION set_otherSignals_RED: Set North South East Lights to RED");
        currentContext.SetSignal("NORTH",false);
        currentContext.SetSignal("SOUTH",false);
        currentContext.SetSignal("EAST",false);
        currentContext.SetSignal("WEST",true);
    }
