...
public class GreenSouth implements StreetLightsStateI
{
    private int carpass_count;
    private StretLights currentContext;
    private Results results;

    public GreenSouth(StretLights contex, Results result)
    {
    ...
    public StreetLightsStateI Set_ALL_Red()
    {
        if ( currentContext.getCurrentState() != currentContext.getAllRed())
        {
            ...
            this.set_otherSignals_RED();
        }
        return currentContext.getAllRed();
    }

    /**
     * Pass a car from south side
     * @return : New state of machine
     */
    @Override
    public StreetLightsStateI pass_car()
    {
        if ( currentContext.getSouth_car_count() > 0 && carpass_count < 2)
        {
            results.storeNewResult("ACTION pass_car: car passing from South side.");
            currentContext.SouthCarPassing();
            carpass_count ++;
        }
    ...
    /**
     * * Set North East West signals to RED
     */
    @Override
    public void set_otherSignals_RED() {
        results.storeNewResult("ACTION set_otherSignals_RED: Set North West East Lights to RED");
        currentContext.SetSignal("NORTH",false);
        currentContext.SetSignal("SOUTH",true);
